...
func (h *FreezeHandler) List(c *gin.Context) {
	...
	filter := repository.FreezeFilter{
		TankID: uint(tankID), Status: c.Query("status"), From: from, To: to,
...
func (h *FreezeHandler) Approve(c *gin.Context) {
	h.decide(c, true)
}

func (h *FreezeHandler) Reject(c *gin.Context) {
	h.decide(c, false)
}

func (h *FreezeHandler) decide(c *gin.Context, approve bool) {
	id, ok := parseID(c, "id")
	if !ok {
		return
	}
	...
	var request dto.DecideFreezeRequest
	if !bindJSON(c, &request) {
		return
	}
	var (
	...
	if approve {
		item, err = h.service.Approve(c.Request.Context(), id, request, actor)
	} else {
		item, err = h.service.Reject(c.Request.Context(), id, request, actor)
	}
	if err != nil {
		api.Fail(c, err)
		return
